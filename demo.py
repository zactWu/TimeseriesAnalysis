...
for i in range(0, 13):
    print('rmse value for', cols[i], 'is : ', sqrt(mean_squared_error(pred.iloc[i], valid.iloc[i])),
          ' mape value for', cols[i], 'is : ', mean_absolute_percentage_error(valid.iloc[i], pred.iloc[i]))

print("a", valid.loc[:, ["PT08.S1(CO)"]])
...
# 可视化
plt.figure()
plt.plot(valid.loc[:'2005/2/1,0:00:00', ["PT08.S1(CO)"]], label='GroundTruth')
plt.plot(pred.loc[:'2005/2/1,0:00:00', ["PT08.S1(CO)"]], label='Prediction')
plt.legend()
plt.show()
